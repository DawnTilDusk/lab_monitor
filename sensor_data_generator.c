...
int main(int argc, char **argv) {
    ...
    for (;;) {
        ...
        int n = snprintf(json, MAX_JSON_SIZE,
                         "{\"device_id\":\"%s\",\"timestamp_ms\":%lld,\"temperature_c\":%.1f,\"light\":%d,\"frame\":{\"width\":%d,\"height\":%d,\"pixels\":"",
                         device_id, ts_ms, temp_c, light, size, size);
        if (n < 0) { free(frame); free(json); break; }
        ...
        int m = snprintf(json + pos, MAX_JSON_SIZE - pos,
